...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const showBanner = () => {
        policyBanner.classList.add('show');
        policyBanner.style.display = 'block'; // Show the banner
    };

    // Check if the policy has already been accepted
    if (!localStorage.getItem('cookiePolicyAccepted')) {
        // Show the banner after a delay (e.g., 1 second)
        setTimeout(showBanner, 1000);
    } else {
        policyBanner.style.display = 'none'; // Hide the banner if already accepted
    }

    // Accept button functionality
    acceptPolicyBtn.addEventListener('click', () => {
        policyBanner.classList.remove('show'); // Remove the show class
        policyBanner.classList.add('hide'); // Add hide class to trigger animation

        // Wait for the animation to finish before hiding the banner
        policyBanner.addEventListener('animationend', () => {
            policyBanner.style.display = 'none'; // Hide the banner
            policyBanner.classList.remove('hide'); // Clean up the class

            // Set a flag in localStorage to remember acceptance
            localStorage.setItem('cookiePolicyAccepted', 'true'); // Set acceptance in localStorage
